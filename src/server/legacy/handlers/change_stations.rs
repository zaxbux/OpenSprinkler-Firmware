use crate::{
    opensprinkler::{gpio, station, Controller},
    server::legacy::error,
};
...
///
/// URI: `/cs`
pub async fn handler(open_sprinkler: web::Data<sync::Arc<sync::Mutex<Controller>>>, parameters: web::Query<HashMap<String, String>>, req: HttpRequest) -> Result<impl Responder> {
    ...
    // disable
    change_stations_attrib(open_sprinkler.config.get_board_count(), &parameters, 'd', |station_index, value| {
    ...
    // special
    change_stations_attrib(open_sprinkler.config.get_board_count(), &parameters, 'p', |station_index, value| {
    ...
    if let Ok(special) = web::Query::<ChangeSpecialStationRequest>::from_query(req.query_string()) {
        if special.station_index > open_sprinkler.config.get_station_count() {
        ...
        if special.station_type == station::StationType::GPIO as u8 {
            if let Ok(data) = <station::GPIOStationData as station::TryFromLegacyString>::try_from_legacy_string(&special.data) {
                ...
                if gpio::is_pin_reserved(&data.pin) {
                    return Ok(error::ReturnErrorCode::DataOutOfBound);
                }
    ...
    let board_count = open_sprinkler.config.get_board_count();

    // master1
    change_stations_attrib(board_count, &parameters, 'm', |station_index, value| {
    ...
    // master2
    change_stations_attrib(board_count, &parameters, 'n', |station_index, value| {
    ...
    // ignore rain
    change_stations_attrib(board_count, &parameters, 'i', |station_index, value| {
    ...
    // ignore sensor1
    change_stations_attrib(board_count, &parameters, 'j', |station_index, value| {
    ...
    // ignore sensor2
    change_stations_attrib(board_count, &parameters, 'k', |station_index, value| {
    ...
    // sequential
    change_stations_attrib(board_count, &parameters, 'q', |station_index, value| {
