...
void server_json_options_main()
{
	...
	{
		// do not send the following parameters for non-Arduino platforms
...
void handle_web_request(char *p)
{
	...
	else
	{
		...
		{
			if (pgm_read_byte(_url_keys + 2 * i) == com[0] && pgm_read_byte(_url_keys + 2 * i + 1) == com[1])
			{
