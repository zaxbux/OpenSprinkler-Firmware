...
pub fn list(open_sprinkler: &Controller) {
    let mut config = open_sprinkler.config.clone();
    config.stations = Vec::new();
    println!("Config: {}", config);
}

pub fn set(config_set: Vec<String>, open_sprinkler: &mut Controller) -> Result<(), &'static str> {
...
pub fn reset(open_sprinkler: &Controller) {
