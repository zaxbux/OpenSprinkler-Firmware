...
impl OSMqtt {
    pub fn init(&mut self) {
        let id = "OS";
        self.client = Some(
            mqtt::AsyncClient::new(mqtt::CreateOptionsBuilder::new().client_id(id).finalize())
    ...
    pub fn begin(
        ...
        let connect_opts = mqtt::ConnectOptionsBuilder::new()
            ...
            .server_uris(&[format!("tcp://{}:{}", host, port).to_string()])
            .user_name(username)
            .password(password)
            .clean_session(true)
        ...
        S: serde::Serialize,
    {
        ...
        let tok =
            self.client
                .as_ref()
                .unwrap()
                .publish(mqtt::Message::new(topic, payload, 0));
        tok.wait().expect("MQTT Publish: Failed");
    }
    //pub fn r#loop() {}
